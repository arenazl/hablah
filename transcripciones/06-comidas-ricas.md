...
## Medición real (infra)
- **Setup** (`POST /sessions/start`): 199 ms
- **Latencia del coach** (1er chunk, Gemini Live native-audio): prom 3.5s · máx 4.6s

## Las 4 patas

### 1) TÓPICO — qué se habla (agnóstico al nivel)
**"Comidas ricas"** · segmento: mini · audiencia: niños. El tópico es solo el tema; no trae vocabulario ni reglas.
...
EL MUNDO DE HOY: "Comidas ricas".
PALABRAS en inglés para tejer en la historia (anclas, NO una lista para recitar): 3-5 palabras simples y visuales de "Comidas ricas" (las elegís vos, del tema).

CÓMO SE LLEVA LA CLASE (narrativa de espina — no negociable):
- Armá una MINI-AVENTURA / cuento sobre "Comidas ricas" donde el chico es el PROTAGONISTA y decide qué pasa. La historia AVANZA en cada turno.
...
ARRANQUE: saludá a Timo por su nombre en Spanish y ABRÍ la historia de hoy sobre "Comidas ricas" con un gancho (algo que pasa, un problemita divertido). Esperá que responda.
```

## Transcripción (clase por la infra real)
```
HABI [apertura · 3.9s]: ¡Hola, Timo! Hoy somos cocineros en una aventura y atraparemos un SANDWICH. A ver, para que no escape, ¿podés repetir conmigo: SANDWICH?
