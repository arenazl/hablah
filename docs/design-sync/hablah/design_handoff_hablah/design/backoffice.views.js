...
function renderNav(sec,tab){
  ...
  document.getElementById('nav').innerHTML=groups.map(g=>{
    ...
    const open=!NAVCLOSED.has(g.name)||hasActive;
  ...
  <div class="side-section"><div class="label">Herramientas</div></div>
  <a class="item" href="Probador_Funcional.html" target="_blank">${ico('probador')}Probador en pantalla completa<span class="ext">↗</span></a>
  <a class="item" href="Kids.html" target="_blank">${ico('alumnos')}Modo Kids<span class="ext">↗</span></a>
...
function toggleGroup(name){if(NAVCLOSED.has(name))NAVCLOSED.delete(name);else NAVCLOSED.add(name);const r=route();renderNav(r[0],r[1])}
