...
/**
 * NOTA libreria core: este componente es AGNOSTICO.
 * Panel "Preguntar a IA" se activa con `showAIButton=true` + callback
 * inyectable `onAskAI(context) => Promise<string>`. NO importes APIs
 * de cliente aca; pasalas como prop desde el wrapper.
 */
export type AskAIFn = (context: { datos: any; tipo: string }) => Promise<string>;
...
interface WizardFormProps {
  ...
  /** Callback inyectable: ejecuta la consulta a IA y devuelve la respuesta.
   * Si no se pasa, el botón "Preguntar a IA" no aparece. */
  onAskAI?: AskAIFn;
}
...
export function WizardForm({
  ...
  // Función para consultar IA — usa callback inyectable `onAskAI`.
  const handleAskAI = async () => {
    ...
    if (!onAskAI) {
      setAiError('Función de consulta a IA no configurada');
      setAiModalOpen(true);
      return;
    }
    ...
    try {
      const response = await onAskAI({ datos: aiContext.datos, tipo: aiContext.tipo });
      setAiResponse(response || 'No se recibió respuesta');
    } catch (error) {
